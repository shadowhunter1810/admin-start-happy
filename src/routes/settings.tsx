  ...
  head: () => ({
    meta: [
      { title: "Settings · Atlas CRM" },
      {
        name: "description",
        content:
          "Account configuration, workflow, notifications, custom fields, integrations, automation and admin actions.",
      },
    ],
  }),
  component: () => (
...
function Pill({ children, tone = "neutral" }: { children: React.ReactNode; tone?: Tone }) {
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-medium ring-1 ${toneMap[tone]}`}
    >
      {children}
    </span>
  );
...
function Btn({
  ...
  return (
    <button
      onClick={onClick}
      className={`inline-flex items-center gap-1 rounded-md px-2.5 py-1.5 text-xs font-medium ring-1 transition ${t[tone]}`}
    >
      {children}
    </button>
  );
...
const SETTINGS: Setting[] = [
  // Account Config
  {
    ...
    value: "VIP Client Template",
    prev: "Standard",
    changedBy: "CRM Admin",
    reason: "Upgraded to VIP",
  ...
  },
  {
    ...
    prev: "Standard",
    changedBy: "Support Manager",
    effective: "12 Jun 2026",
    valueTone: "success",
  },
  {
    key: "account_workflow",
  ...
  // Report
  {
    key: "report_type",
    label: "Report Type",
    group: "Report Preferences",
    value: "Trading Summary",
  },
...
const AUTOMATIONS = [
  {
    rule: "VIP Assignment — IF Deposit > $100,000 THEN Assign VIP Manager",
    status: "Active",
    tone: "success" as Tone,
  },
  {
    rule: "Dormant Reminder — IF inactive > 60d THEN Send re-engagement email",
    status: "Active",
    tone: "success" as Tone,
  },
  {
    rule: "Statement Generation — Monthly trading statement to client",
    status: "Active",
    tone: "success" as Tone,
  },
  {
    rule: "High Risk Alert — IF risk score > 80 THEN Notify Risk Team",
    status: "Paused",
    tone: "warn" as Tone,
  },
];
        ...
        <nav className="flex flex-col">
          {SECTIONS.map((s) => (
            <button
              ...
              className={`rounded-md px-2.5 py-1.5 text-left text-xs transition ${active === s.k ? "bg-slate-900 text-white" : "text-slate-700 hover:bg-slate-50"}`}
            >
              {s.label}
            </button>
          ))}
      ...
      <div className="space-y-5">
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs text-slate-500">
                System-level account behaviour, workflow, preferences, custom fields, integrations
                and automations.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              <Btn tone="info" onClick={() => toast.success("Settings exported")}>
                Export Settings
              </Btn>
              <Btn tone="success" onClick={() => toast.success("Template applied")}>
                Apply Template
              </Btn>
              <Btn tone="warn" onClick={() => toast.success("Custom field added")}>
                Add Custom Field
              </Btn>
            </div>
          </div>
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              <div className="rounded-lg border border-slate-200 p-3 text-sm">
                <div className="mb-1 text-xs font-semibold text-slate-700">
                  Final Separation (Enterprise Correct)
                </div>
                <div className="grid grid-cols-2 gap-x-3 text-xs">
                  {[
                    ["Name, Email, Country", "Overview"],
                    ["Balance, Deposit, Withdrawal", "Financial"],
                    ["MT5 Account, Leverage", "Trading Accounts"],
                    ["Trading Permission", "Permissions"],
                    ["Login Security", "Security & Sessions"],
                    ["KYC Data", "KYC & Compliance"],
                    ["Commission", "Commissions"],
                    ["Preferences & System Config", "Settings"],
                  ].map(([info, tab]) => (
                    <div
                      key={info}
                  ...
                  {SETTINGS.filter((s) => s.prev).map((s) => (
                    ...
                    >
                      ...
                      <span className="flex items-center gap-1">
                        <Pill>{s.prev}</Pill>→<Pill tone="success">{s.value}</Pill>
                      </span>
                    </li>
                  ))}
                </ul>
        ...
        {active === "notifications" && (
          <GroupTable
            ...
            subtitle="Future communication rules — not history."
            rows={byGroup("Notifications")}
            onOpen={setDrawer}
          />
        )}
        {active === "language" && (
          <GroupTable
            title="5. Language & Localization"
            subtitle="User experience settings."
            rows={byGroup("Language & Localization")}
            onOpen={setDrawer}
          />
        )}
        {active === "privacy" && (
          <GroupTable
            title="6. Data & Privacy Preferences"
            subtitle="Data usage preferences."
            rows={byGroup("Data & Privacy")}
            onOpen={setDrawer}
          />
        )}
        ...
        {active === "custom" && (
          <Section
            title="7. Custom Fields"
            subtitle="Enterprise-specific extra information — every broker has different requirements."
            right={
              <Btn tone="violet" onClick={() => toast.success("Custom field added")}>
                + Add Field
              </Btn>
            }
          >
                  ...
                  {CUSTOM_FIELDS.map((c) => (
                    <tr key={c.field} className="border-t border-slate-100">
                      ...
                      <td className="px-3 py-2">
                        <Pill tone="violet">{c.value}</Pill>
                      </td>
                      <td className="px-3 py-2 text-slate-600">{c.by}</td>
                      <td className="px-3 py-2 text-right">
                        <Btn onClick={() => toast.success("Edited")}>Edit</Btn>
                      </td>
                    </tr>
                  ))}
                </tbody>
                  ...
                  {INTEGRATIONS.map((i) => (
                    <tr key={i.system} className="border-t border-slate-100">
                      ...
                      <td className="px-3 py-2 text-right space-x-1">
                        <Btn onClick={() => toast.success("Reconnected")} tone="info">
                          Reconnect
                        </Btn>
                        <Btn onClick={() => toast.error("Disconnected")} tone="danger">
                          Disconnect
                        </Btn>
                      </td>
                    </tr>
        ...
        {active === "automation" && (
          <Section
            title="9. Automation Rules"
            subtitle="Automatic actions."
            right={
              <Btn tone="success" onClick={() => toast.success("Rule created")}>
                + Create Rule
              </Btn>
            }
          >
                ...
                <tbody>
                  {AUTOMATIONS.map((a) => (
                    <tr key={a.rule} className="border-t border-slate-100">
                      ...
                      <td className="px-3 py-2 text-right space-x-1">
                        <Btn onClick={() => toast.success("Edited")}>Edit</Btn>
                        <Btn onClick={() => toast.error("Disabled")} tone="danger">
                          Disable
                        </Btn>
                      </td>
                    </tr>
        ...
        {active === "ui" && (
          <GroupTable
            title="10. User Interface Preferences"
            subtitle="Personal dashboard customization."
            rows={byGroup("UI Preferences")}
            onOpen={setDrawer}
          />
        )}
        {active === "document" && (
          <GroupTable
            title="11. Document Preferences"
            subtitle="Document delivery settings."
            rows={byGroup("Document Preferences")}
            onOpen={setDrawer}
          />
        )}
        {active === "report" && (
          <GroupTable
            title="12. Report Preferences"
            subtitle="Automatic reports."
            rows={byGroup("Report Preferences")}
            onOpen={setDrawer}
          />
        )}
        ...
        {active === "notes" && (
          <Section title="Notes" subtitle="Internal notes on settings configuration.">
            <textarea
              ...
              rows={4}
              placeholder="Add internal note about settings…"
            />
            <div className="mt-2">
              <Btn tone="info" onClick={() => toast.success("Note saved")}>
                Save Note
              </Btn>
            </div>
          </Section>
        ...
        {active === "actions" && (
          <Section
            title="Admin Actions"
            subtitle="Safe, moderate, dangerous — with permission, reason, confirmation and audit log."
          >
            <div className="grid gap-3 md:grid-cols-3">
              <div className="rounded-lg border border-slate-200 p-3">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {[
                    ...
                    ["Add Note", "Internal record"],
                    ["Compare Changes", "Difference view"],
                  ].map(([a, p]) => (
                    <Btn key={a} tone="success" onClick={() => toast.success(`${a}: ${p}`)}>
                      {a}
                    </Btn>
                  ))}
                </div>
              ...
              <div className="rounded-lg border border-slate-200 p-3">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {[
                    ["Update Configuration", "Change setting"],
                    ["Add Custom Field", "Extend profile"],
                    ["Apply Template", "Apply workflow"],
                    ["Create Automation", "Add rule"],
                  ].map(([a, p]) => (
                    <Btn key={a} tone="warn" onClick={() => toast.success(`${a}: ${p}`)}>
                      {a}
                    </Btn>
                  ))}
                </div>
              ...
              <div className="rounded-lg border border-slate-200 p-3">
                <div className="mb-2 text-xs font-semibold text-rose-700">
                  Dangerous (permission + reason + confirm + log)
                </div>
                <div className="flex flex-wrap gap-1.5">
                  {[
                    ["Reset Settings", "Restore default"],
                    ["Delete Custom Field", "Remove custom data"],
                    ["Disable Automation", "Stop process"],
                    ["Remove Integration", "Disconnect system"],
                  ].map(([a, p]) => (
                    <Btn key={a} tone="danger" onClick={() => toast.error(`${a}: ${p}`)}>
                      {a}
                    </Btn>
                  ))}
                </div>
        ...
        {active === "audit" && (
          <Section
            title="Audit Trail"
            subtitle="All configuration changes logged with who / when / why / before / after."
          >
            <div className="overflow-hidden rounded-lg border border-slate-200">
              <table className="w-full text-sm">
                <thead className="bg-slate-50 text-left text-[11px] uppercase tracking-wide text-slate-500">
                ...
                <tbody>
                  {SETTINGS.filter((s) => s.prev).map((s) => (
                    <tr key={s.key} className="border-t border-slate-100">
                      ...
                      <td className="px-3 py-2">
                        <Pill tone="success">{s.value}</Pill>
                      </td>
          ...
          <tbody>
            {rows.map((r) => (
              ...
              >
                ...
                <td className="px-3 py-2">
                  <Pill tone={r.valueTone ?? "neutral"}>{r.value}</Pill>
                </td>
        ...
        <nav className="flex flex-wrap gap-1 border-b border-slate-200 px-3 py-2">
          {(
            ...
            <button
              key={k}
              onClick={() => setTab(k)}
              className={`rounded px-2 py-1 text-[11px] ${tab === k ? "bg-slate-900 text-white" : "bg-slate-50 text-slate-700 hover:bg-slate-100"}`}
            >
              {l}
            </button>
          ))}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-1 text-sm">
          ...
          {tab === "timeline" && (
            <ol className="relative ml-3 border-l border-slate-200">
              {[
                ["18 Jun", "Updated by Client"],
                ["12 Jun", "Reviewed by CRM Admin"],
                ["01 Jun", "Initial value set"],
              ].map(([t, a]) => (
          ...
          {tab === "automation" && (
            <div className="text-xs text-slate-600">
              ...
              <p className="mt-2">
                If this setting changes, the linked automation re-evaluates the client profile.
              </p>
            </div>
          )}
          {tab === "history" && (
            <ul className="space-y-1">
              <li className="flex items-center justify-between border-b border-slate-100 py-1.5">
                {s.label} updated<Pill tone="success">Completed</Pill>
              </li>
              <li className="flex items-center justify-between border-b border-slate-100 py-1.5">
                {s.label} reviewed<Pill tone="info">Reviewed</Pill>
              </li>
              <li className="flex items-center justify-between border-b border-slate-100 py-1.5">
                {s.label} created<Pill>Initial</Pill>
              </li>
            </ul>
          )}
        </div>
        <footer className="flex flex-wrap gap-2 border-t border-slate-200 p-3">
          <Btn tone="info" onClick={() => toast.success("Compare opened")}>
            Compare Changes
          </Btn>
          <Btn tone="warn" onClick={() => toast.success("Edit mode")}>
            Update Value
          </Btn>
          <Btn tone="danger" onClick={() => toast.error("Reset to default")}>
            Reset
          </Btn>
