  ...
  head: () => ({
    meta: [
      { title: "Marketing Campaigns · Atlas CRM" },
      {
        name: "description",
        content:
          "Client/IB marketing attribution, campaigns, UTM, promotions, referrals, email, SMS & push.",
      },
    ],
  }),
  component: () => (
...
const Btn = ({
  ...
  return (
    <button onClick={onClick} className={`rounded-md px-3 py-1.5 text-xs font-medium ${cls}`}>
      {children}
    </button>
  );
};
...
const UTM_REG = [
  { f: "UTM Source", v: "google" },
  { f: "UTM Medium", v: "cpc" },
  { f: "UTM Campaign", v: "gold-trading-2022" },
  { f: "UTM Content", v: "banner-300x250" },
  { f: "UTM Term", v: "forex trading india" },
  { f: "Landing Page", v: "broker.com/gold-trading" },
  { f: "Device at Registration", v: "Chrome / Android" },
  { f: "Country at Registration", v: "India · Mumbai" },
];

const CAMPAIGNS = [
  {
    id: "CAMP-2026-018",
    name: "May Deposit Promo",
    type: "Deposit bonus",
    joined: "09 May 2026",
    status: "Active",
    deposits: "$2,000",
  ...
  },
  {
    ...
    traders: 48,
    vol: "4,200 lots",
    utm: "vip-deposit-2024",
  },
  {
    id: "CAMP-2022-001",
    name: "Gold Trading 2022",
    type: "Acquisition",
    joined: "14 Mar 2022",
    status: "Completed",
    deposits: "$5,000",
...
const PROMOS = [
  {
    ...
    deposit: "$1,000+",
    aml: "Clear",
    region: "India · allowed",
  },
  {
    id: "PROMO-002",
    name: "Ramadan Cashback 5%",
    type: "Cashback",
    amount: "$480",
    date: "15 Apr 2026",
    status: "Active",
...
const RESTRICTIONS = [
  {
    r: "Self-Referral Block",
    status: "Allow",
    note: "Self-referral detection enabled",
    reason: "Default policy",
  },
  {
    r: "IB Excluded From Bonus",
    status: "Partial",
    note: "IB partners excluded from deposit bonuses but eligible for cashback",
    reason: "IB policy",
  },
...
const NOTES = [
  {
    txt: "High campaign engagement — eligible for VIP promotion.",
    by: "Marketing",
    d: "10 May 2026",
  },
  {
    txt: "Bonus abuse review pending — flagged once for multi-accounting.",
    by: "Compliance",
    d: "28 Apr 2026",
  },
];
        ...
        <div className="mx-auto max-w-7xl px-6 py-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex gap-2">
              <Btn variant="ghost" onClick={() => notify("Exported")}>
                Export
              </Btn>
              <Btn onClick={() => notify("Campaign assigned")}>Assign Campaign</Btn>
              <Btn variant="danger" onClick={() => notify("Bonus revoked")}>
                Revoke Bonus
              </Btn>
            </div>
          </div>
          ...
          <div className="flex gap-1">
            {SECTIONS.map((s) => (
              <button
                key={s.k}
                onClick={() => setTab(s.k)}
                className={`whitespace-nowrap border-b-2 px-3 py-2.5 text-sm transition ${tab === s.k ? "border-gray-900 font-semibold" : "border-transparent text-gray-500 hover:text-gray-800"}`}
              >
                {s.label}
              </button>
            ))}
      ...
      <div className="mx-auto max-w-7xl space-y-6 px-6 py-6">
          ...
          <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
            {KPIS.map((k) => (
              <Card key={k.l} className="p-4">
                ...
                <div className="mt-1 text-xs text-gray-500">{k.s}</div>
              </Card>
            ))}
          </div>
        )}
        ...
        {tab === "attribution" && (
          <div className="grid gap-6 md:grid-cols-2">
            <Section title="Acquisition Attribution">
              {ATTRIBUTION.map((r) => (
                <KV key={r.f} k={r.f} v={r.v} />
              ))}
            </Section>
            <Section title="UTM Parameters (Registration)">
              {UTM_REG.map((r) => (
                <KV key={r.f} k={r.f} v={r.v} />
              ))}
            </Section>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-100">
                <thead className="bg-gray-50">
                  <tr>
                    {["Campaign ID", "Name", "Type", "Joined", "Status", "Deposits", "Revenue"].map(
                      (c) => (
                        <Th key={c}>{c}</Th>
                      ),
                    )}
                ...
                <tbody className="divide-y divide-gray-100">
                  {CAMPAIGNS.map((r) => (
                    <tr
                      ...
                      onClick={() => setDrawer({ kind: "campaign", data: r })}
                    >
                      ...
                      <Td className="font-semibold">{r.deposits}</Td>
                      <Td>{r.revenue}</Td>
                    </tr>
                  ))}
                </tbody>
        ...
        {tab === "utm" && (
          <Section
            title="UTM Tracking"
            subtitle="Marketing attribution tracking · Click for details"
          >
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-100">
                <thead className="bg-gray-50">
                ...
                <tbody className="divide-y divide-gray-100">
                  {UTM_TABLE.map((r) => (
                    <tr
                      key={r.utm}
                      className="cursor-pointer hover:bg-gray-50"
                      onClick={() => setDrawer({ kind: "utm", data: r })}
                    >
                      <Td className="font-medium">{r.utm}</Td>
                      <Td>{r.source}</Td>
                      <Td>{r.medium}</Td>
                      <Td>{r.clicks}</Td>
                      <Td>{r.regs}</Td>
                      <Td>{r.deposits}</Td>
                      <Td className="font-semibold">{r.revenue}</Td>
                      <Td className={r.roi.includes("+") ? "text-emerald-600 font-semibold" : ""}>
                        {r.roi}
                      </Td>
                    </tr>
                  ))}
        ...
        {tab === "promo" && (
          <Section
            ...
            subtitle="Click row for eligibility & usage details"
          >
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-100">
                <thead className="bg-gray-50">
                  <tr>
                    {["Promo ID", "Name", "Type", "Amount", "Date", "Status"].map((c) => (
                      <Th key={c}>{c}</Th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {PROMOS.map((r) => (
                    <tr
                      key={r.id}
                      className="cursor-pointer hover:bg-gray-50"
                      onClick={() => setDrawer({ kind: "promo", data: r })}
                    >
                      <Td className="font-medium">{r.id}</Td>
                      <Td>{r.name}</Td>
                      <Td>{r.type}</Td>
                ...
                <div key={r.id}>
                  <div className="mb-3 grid grid-cols-2 gap-3 md:grid-cols-5">
                    <Card className="p-3">
                      ...
                      <div className="text-xl font-semibold">{r.totalRefs}</div>
                    </Card>
                    <Card className="p-3">
                      <div className="text-xs text-gray-500">KYC Verified</div>
                      <div className="text-xl font-semibold">{r.verified} (66.7%)</div>
                    </Card>
                    <Card className="p-3">
                      <div className="text-xs text-gray-500">Depositors</div>
                      <div className="text-xl font-semibold">{r.depositors} (73.5%)</div>
                    </Card>
                    <Card className="p-3">
                      <div className="text-xs text-gray-500">Active Traders</div>
                      <div className="text-xl font-semibold">{r.traders} (52.8%)</div>
                    </Card>
                    <Card className="p-3">
                      <div className="text-xs text-gray-500">VIP Clients</div>
                      <div className="text-xl font-semibold">{r.vip} (10.5%)</div>
                    </Card>
                  </div>
          ...
          <Section title="Email Campaign History">
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-100">
                <thead className="bg-gray-50">
                  <tr>
                    {["Campaign", "Sent", "Opened", "Clicked", "Status"].map((c) => (
                      <Th key={c}>{c}</Th>
                    ))}
                  </tr>
                </thead>
          ...
          <Section title="SMS Campaign History">
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-100">
                <thead className="bg-gray-50">
                  <tr>
                    {["Campaign", "Sent", "Delivered", "Clicked"].map((c) => (
                      <Th key={c}>{c}</Th>
                    ))}
                  </tr>
                </thead>
          ...
          <Section title="Push Notification Campaigns">
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-100">
                <thead className="bg-gray-50">
                  <tr>
                    {["Notification", "Sent", "Opened", "Action Taken"].map((c) => (
                      <Th key={c}>{c}</Th>
                    ))}
                  </tr>
                </thead>
              ...
              {FUNNEL.map((f, i) => (
                <div key={f.s} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 rounded bg-gray-100">
                    <div
                      className={`rounded py-2 pl-3 text-xs font-semibold text-white ${f.v ? "bg-emerald-500" : "bg-gray-300"}`}
                      style={{ width: f.v ? `${100 - i * 8}%` : "6%" }}
                    >
                      {f.v ? "Completed" : "Not reached"}
                    </div>
                  </div>
                </div>
              ))}
          ...
          <Section title="Restrictions & Eligibility" subtitle="Campaign rule enforcement">
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-100">
                <thead className="bg-gray-50">
                  <tr>
                    {["Restriction", "Status", "Note", "Reason"].map((c) => (
                      <Th key={c}>{c}</Th>
                    ))}
                  </tr>
                </thead>
        ...
        {tab === "actions" && (
          <div className="grid gap-6 md:grid-cols-3">
            <Section title="Safe Actions">
              <div className="space-y-2">
                {["View Campaigns", "Export Report", "Add Note", "View Promotions"].map((a) => (
                  <Btn key={a} variant="ghost" onClick={() => notify(a)}>
                    {a}
                  </Btn>
                ))}
              </div>
            </Section>
            <Section title="Moderate Actions">
              <div className="space-y-2">
                {["Assign Campaign", "Remove Campaign", "Grant Bonus", "Revoke Bonus"].map((a) => (
                  <Btn key={a} onClick={() => notify(a)}>
                    {a}
                  </Btn>
                ))}
              </div>
            ...
            <Section
              ...
              subtitle="Permission + reason + audit log + confirmation"
            >
              <div className="space-y-2">
                {["Ban Bonus Participation", "Revoke Rewards", "Block from Campaigns"].map((a) => (
                  <Btn key={a} variant="danger" onClick={() => notify(a)}>
                    {a}
                  </Btn>
                ))}
              </div>
          ...
          <Section title="Audit Trail">
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-100">
                <thead className="bg-gray-50">
                  <tr>
                    {["Action", "By", "Entity", "Timestamp"].map((c) => (
                      <Th key={c}>{c}</Th>
                    ))}
                  </tr>
                </thead>
...
function MarketingDrawer({
  ...
  const tabs =
    kind === "campaign"
      ? ["overview", "participation", "analytics", "related", "timeline", "audit"]
      : kind === "utm"
        ? ["overview", "traffic", "conversion", "geo", "audit"]
        : kind === "promo"
          ? ["overview", "eligibility", "usage", "fraud", "audit"]
          : ["overview", "details", "timeline"];

  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-gray-900/40" onClick={onClose}>
      <div
        className="h-full w-full max-w-2xl overflow-y-auto bg-white"
      ...
      >
          ...
          <div className="mt-3 flex flex-wrap gap-1 -mb-5 border-b">
            {tabs.map((x) => (
              <button
                key={x}
                onClick={() => setT(x)}
                className={`border-b-2 px-3 py-2 text-xs capitalize ${t === x ? "border-gray-900 font-semibold" : "border-transparent text-gray-500"}`}
              >
                {x}
              </button>
            ))}
        ...
        <div className="space-y-2 p-5">
          {kind === "campaign" && t === "overview" && (
            <>
              <KV k="Campaign ID" v={r.id} />
              <KV k="Name" v={r.name} />
              <KV k="Type" v={r.type} />
              <KV k="Owner" v={r.owner} />
              <KV k="Start" v={r.start} />
              <KV k="End" v={r.end} />
          ...
          {kind === "campaign" && t === "participation" && (
            <>
              ...
              <KV k="Referral Code" v="AR4120" />
              <KV k="Tracking Link" v="broker.com/r/AR4120" />
              <KV k="Registration Source" v="Google CPC" />
              <KV k="UTM Campaign" v={r.utm} />
            </>
          )}
          {kind === "campaign" && t === "analytics" && (
            <>
              <KV k="Impressions" v={r.impressions?.toLocaleString()} />
              <KV k="Clicks" v={r.clicks?.toLocaleString()} />
              <KV k="Registrations" v={r.regs} />
              <KV k="KYC Approved" v={r.kyc} />
              <KV k="First Deposits" v={r.fd} />
              <KV k="Active Traders" v={r.traders} />
              <KV k="Trading Volume" v={r.vol} />
              <KV k="Revenue Generated" v={r.revenue} />
            </>
          )}
          {kind === "campaign" && t === "related" && (
            <>
              <KV k="Related Accounts" v="MT5-00421, MT5-00298" />
              <KV k="Related Clients" v="142 clients" />
              <KV k="Related IBs" v="IB-00412 · Arjun Raghunathan" />
              <KV k="Related Affiliates" v="Google Ads Network" />
            </>
          )}
          ...
          {kind === "utm" && t === "overview" && (
            <>
              <KV k="UTM Campaign" v={r.utm} />
              <KV k="Source" v={r.source} />
              <KV k="Medium" v={r.medium} />
              <KV k="Clicks" v={r.clicks} />
              <KV k="Registrations" v={r.regs} />
              <KV k="Revenue" v={r.revenue} />
              <KV k="ROI" v={r.roi} />
            </>
          ...
          {kind === "utm" && t === "traffic" && (
            <>
              <KV k="Unique Visitors" v="3,940" />
              <KV k="Bounce Rate" v="38%" />
              <KV k="Avg Time on Page" v="2m 14s" />
              <KV k="Top Landing Page" v="broker.com/gold-trading" />
            </>
          )}
          {kind === "utm" && t === "conversion" && (
            <>
              <KV k="Reg → KYC" v="72%" />
              <KV k="KYC → Deposit" v="63%" />
              <KV k="Deposit → Trade" v="78%" />
              <KV k="Lead → VIP" v="0.8%" />
            </>
          )}
          {kind === "utm" && t === "geo" && (
          ...
          {kind === "promo" && t === "eligibility" && (
            <>
              <KV k="KYC Requirement" v={<span>{r.kyc} ✓</span>} />
              <KV k="Deposit Requirement" v={<span>{r.deposit} ✓</span>} />
              <KV k="AML Requirement" v={<span>{r.aml} ✓</span>} />
              <KV k="Region Eligibility" v={<span>{r.region} ✓</span>} />
            </>
          )}
          {kind === "promo" && t === "usage" && (
            <>
              <KV k="Times Used" v="1" />
              <KV k="Total Granted" v={r.amount} />
              <KV k="Total Wagered" v="—" />
              <KV k="Conversion" v="Deposited & traded" />
            </>
          )}
          {kind === "promo" && t === "fraud" && (
            <>
              <KV k="Device Sharing Risk" v={<Pill tone="green">Clear</Pill>} />
              <KV k="Self Referral Risk" v={<Pill tone="green">Clear</Pill>} />
              <KV k="Bonus Abuse" v={<Pill tone="green">Clear</Pill>} />
            </>
          )}
          ...
          {t === "timeline" && (
            <ol className="space-y-2 text-sm">
              <li className="border-l-2 border-gray-300 pl-3">
                <div className="text-xs text-gray-500">Today · System</div>
                <div>Record viewed</div>
              </li>
              <li className="border-l-2 border-gray-300 pl-3">
                <div className="text-xs text-gray-500">{r.date || r.joined} · Marketing</div>
                <div>Created / assigned</div>
              </li>
            </ol>
          )}
          {t === "audit" && (
            <ol className="space-y-1 text-xs text-gray-600">
              <li>Created · System · {r.date || r.joined}</li>
              <li>Updated · Marketing · Today</li>
            </ol>
          )}
        </div>
        <div className="sticky bottom-0 flex justify-end gap-2 border-t bg-white p-4">
          ...
          <Btn variant="ghost" onClick={() => notify("Flagged")}>
            Flag
          </Btn>
