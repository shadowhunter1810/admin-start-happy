  ...
  head: () => ({
    meta: [
      { title: "IB / Partner · Atlas CRM" },
      {
        name: "description",
        content:
          "Introducing Broker partner dashboard — profile, tier, commissions, referrals, sub-IBs, risk, compliance, audit.",
      },
    ],
  }),
  component: () => (
...
const PARTNER = {
  ...
  status: "Active",
  tier: "Gold IB",
  tierLevel: "Tier 3 of 6",
  referralLink: "broker.com/r/AR4120",
  landing: "broker.com/ib/ar-financial",
  utm: "google / cpc",
  trackingId: "TRK-AR4120",
  clicks: 4820,
  regs: 147,
  conversion: "3.05%",
};
    ...
    <div className="grid gap-4 lg:grid-cols-3">
      ...
      <Section title="Referral Links">
        ...
        <div className="mt-3 flex gap-2">
          <Btn variant="outline" onClick={() => toast.success("Link copied")}>
            Copy Link
          </Btn>
        </div>
      </Section>
      <Section title="QR Code">
        <div className="flex flex-col items-center gap-3 py-2">
          ...
          <Btn variant="outline" onClick={() => toast.success("QR downloaded")}>
            Download QR
          </Btn>
        </div>
      </Section>
      ...
      <Section title="Partner Types" className="lg:col-span-3">
        <div className="flex flex-wrap gap-2">
          {[
            ...
            <Pill key={t} tone={t === PARTNER.type ? "violet" : "gray"}>
              {t}
            </Pill>
          ))}
        </div>
      ...
      <Section title="IB Tier Ladder">
        <div className="space-y-2">
          {tiers.map((t) => (
            <div
              key={t.name}
              className={`flex items-center justify-between rounded-md border p-3 text-sm ${t.current ? "border-amber-300 bg-amber-50" : "border-gray-200"}`}
            >
              <div className="flex items-center gap-2">
                <span className="font-medium">{t.name}</span>
                {t.current && <Pill tone="amber">← Current</Pill>}
              </div>
      ...
      <Section title="Risk Levels">
        {["Low", "Medium", "High", "Critical"].map((l) => (
          ...
          >
            ...
            <Pill tone={l === "Low" ? "green" : l === "Medium" ? "amber" : "red"}>
              {l === "Medium" ? "Current" : "—"}
            </Pill>
          </div>
        ))}
      ...
      <Section title="Active Alerts">
        <div className="space-y-2 text-sm">
          <div className="rounded-md bg-amber-50 p-2 text-amber-700">
            ⚠ Elevated chargeback ratio (1.4%)
          </div>
          <div className="rounded-md bg-emerald-50 p-2 text-emerald-700">
            ✓ No fake-leads detected last 30d
          </div>
        </div>
      </Section>
    </div>
...
function ReferralNetwork({ onOpen }: { onOpen: (c: { id: string; name: string }) => void }) {
  return (
    <Section
      ...
      subtitle="Click any row to open client drawer"
      action={
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="border-b border-gray-200 bg-gray-50 text-left text-[11px] uppercase tracking-wide text-gray-500">
            <tr>
              {["Client ID", "Name", "Country", "Status", "Deposits", "Volume", "Revenue", ""].map(
                (h) => (
  ...
  return (
    <Section title="Sub-IB Network" subtitle="Downline IBs registered under this partner">
      <table className="w-full text-sm">
        <thead className="border-b border-gray-200 bg-gray-50 text-left text-[11px] uppercase tracking-wide text-gray-500">
          <tr>
            {["Sub-IB ID", "Name", "Clients", "Volume", "Commission", "Status"].map((h) => (
              <th key={h} className="px-3 py-2 font-medium">
                {h}
              </th>
            ))}
        ...
        <tbody>
          {subs.map((s) => (
            <tr key={s.id} className="border-b border-gray-100">
              ...
              <td className="px-3 py-2">
                <Pill tone={s.status === "Active" ? "green" : "gray"}>{s.status}</Pill>
              </td>
            </tr>
          ))}
        </tbody>
  ...
  return (
    <Section title="Payment History">
      <table className="w-full text-sm">
        <thead className="border-b border-gray-200 bg-gray-50 text-left text-[11px] uppercase tracking-wide text-gray-500">
          <tr>
            {["Payment ID", "Amount", "Method", "Currency", "Date", "Status"].map((h) => (
              <th key={h} className="px-3 py-2 font-medium">
                {h}
              </th>
            ))}
        ...
        <tbody>
          {pays.map((p) => (
            <tr key={p.id} className="border-b border-gray-100">
              ...
              <td className="px-3 py-2">
                <Pill
                  tone={p.status === "Paid" ? "green" : p.status === "Pending" ? "amber" : "red"}
                >
                  {p.status}
                </Pill>
              </td>
  ...
  return (
    <Section title="Commission Adjustments">
      <table className="w-full text-sm">
        <thead className="border-b border-gray-200 bg-gray-50 text-left text-[11px] uppercase tracking-wide text-gray-500">
          <tr>
            {["ID", "Amount", "Reason", "Changed By", "Date"].map((h) => (
              <th key={h} className="px-3 py-2 font-medium">
                {h}
              </th>
            ))}
        ...
        <tbody>
          {adj.map((a) => (
            <tr key={a.id} className="border-b border-gray-100">
              ...
              <td
                className={`px-3 py-2 font-medium ${a.amount.startsWith("+") ? "text-emerald-600" : "text-rose-600"}`}
              >
                {a.amount}
              </td>
      ...
      <div className="mt-3 text-xs text-gray-500">
        Reasons: Commission Correction · Fraud Reversal · Duplicate Payment · Manual Bonus · Manager
        Override
      </div>
    </Section>
  );
  ...
  return (
    <Section title="Marketing Resources">
      <table className="w-full text-sm">
        <thead className="border-b border-gray-200 bg-gray-50 text-left text-[11px] uppercase tracking-wide text-gray-500">
          <tr>
            {["Resource", "Status", "Target", "Engagement"].map((h) => (
              <th key={h} className="px-3 py-2 font-medium">
                {h}
              </th>
            ))}
        ...
        <tbody>
          {items.map((i) => (
            <tr key={i.name} className="border-b border-gray-100">
              ...
              <td className="px-3 py-2">
                <Pill tone={i.status === "Active" || i.status === "Approved" ? "green" : "gray"}>
                  {i.status}
                </Pill>
              </td>
    ...
    <Section title="Risk Monitoring">
      <div className="grid gap-2 md:grid-cols-2">
        {alerts.map((a) => (
          <div
            key={a.name}
            className={`flex items-center justify-between rounded-md border p-3 text-sm ${a.active ? "border-rose-200 bg-rose-50" : "border-gray-200"}`}
          >
            <span>{a.name}</span>
  ...
  return (
    <Section title="Restrictions & Controls">
      <table className="w-full text-sm">
        <thead className="border-b border-gray-200 bg-gray-50 text-left text-[11px] uppercase tracking-wide text-gray-500">
          <tr>
            {["Restriction", "Status", "Reason", "Applied By", "Action"].map((h) => (
              <th key={h} className="px-3 py-2 font-medium">
                {h}
              </th>
            ))}
        ...
        <tbody>
          {rs.map((r) => (
            <tr key={r.name} className="border-b border-gray-100">
              ...
              <td className="px-3 py-2">
                <Btn variant="outline" onClick={() => toast(`Hold applied to ${r.name}`)}>
                  Hold
                </Btn>
              </td>
            </tr>
        ...
        {events.map((e, i) => (
          <li key={i} className="flex gap-3">
            <div className="flex flex-col items-center">
              <div className="h-2 w-2 rounded-full bg-gray-900 mt-1.5" />
...
function Notes() {
  const notes = [
    {
      tag: "VIP Partner",
      text: "Strategic partner — extend custom plan after Q2 review.",
      by: "Priya Nair",
      at: "12 May 2026",
    },
    {
      tag: "High Performing IB",
      text: "Consistent monthly growth >10%.",
      by: "Finance",
      at: "02 May 2026",
    },
    {
      tag: "Requires Custom Commission",
      text: "Renegotiation pending.",
      by: "Manager",
      at: "28 Apr 2026",
    },
    {
      tag: "Under Compliance Review",
      text: "Annual KYC refresh scheduled.",
      by: "Rajan Mehta",
      at: "08 May 2026",
    },
  ];
...
function AdminActions() {
  const group = (
    ...
    <Section title={title}>
      <div className="flex flex-wrap gap-2">
        {items.map((i) => (
          <Btn key={i} variant={variant} onClick={() => toast(`${i} requested`)}>
            {i}
          </Btn>
        ))}
      </div>
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-3">
      {group(
        "Safe Actions",
        ["View Profile", "Notify Partner", "Export Report", "Add Note"],
        "outline",
      )}
      {group(
        "Moderate Actions",
      ...
      <Section title="Permissions & Confirmation" className="lg:col-span-3">
        <div className="text-xs text-gray-500">
          Dangerous actions require: permission · confirmation · reason · audit log entry.
        </div>
      </Section>
    </div>
  );
...
function Audit() {
  const rows = [
    ...
    {
      a: "Commission plan updated to Plan B",
      by: "Priya Nair",
      at: "04 Apr 2024 16:21",
      ip: "10.0.4.21",
    },
    {
      a: "Payment PAY-7821 marked Paid",
      by: "Finance Bot",
      at: "01 May 2026 09:00",
      ip: "10.0.0.2",
    },
    {
      a: "Note added: Under Compliance Review",
      by: "Rajan Mehta",
      at: "08 May 2026 12:14",
      ip: "10.0.4.55",
    },
    {
      a: "Restriction reviewed — none active",
      by: "Compliance",
      at: "08 May 2026 12:30",
      ip: "10.0.4.55",
    },
  ];
  return (
...
function ClientDrawer({
  ...
  const [tab, setTab] = useState<
    "overview" | "trading" | "financial" | "risk" | "timeline" | "actions"
  >("overview");
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="flex h-full w-full max-w-xl flex-col border-l border-gray-200 bg-white shadow-xl">
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {tab === "overview" && (
            <>
              <KV k="Total Volume" v="2,840 lots" />
              <KV k="Active Positions" v="4" />
              <KV k="Preferred Instrument" v="EUR/USD" />
              <KV k="Avg Trade Size" v="0.8 lots" />
              <KV k="Win Rate" v="58%" tone="green" />
            </>
          )}
          {tab === "trading" && (
            <>
              <KV k="Total Lots" v="2,840" />
              <KV k="Total Trades" v="284" />
              <KV k="Win Rate" v="58%" tone="green" />
              <KV k="Avg Trade Size" v="0.8 lots" />
              <KV k="Best Month" v="$4,200 (Mar)" />
              <KV k="Instruments" v="EURUSD, XAUUSD, WTI" />
              <KV k="Copy Trading" v="No" />
              <KV k="Last Trade" v="Today" />
            </>
          )}
          {tab === "financial" && (
            <>
              <KV k="Total Deposits" v="$48,200" tone="green" />
              <KV k="Revenue Generated" v="$3,120" tone="green" />
              <KV k="Commission Earned" v="$3,120" />
              <KV k="Last Deposit" v="14 May 2026" />
              <KV k="Bonus Used" v="$0" />
            </>
          )}
          {tab === "risk" && (
            <>
              <KV k="Risk Level" v={<Pill tone="green">Low</Pill>} />
          ...
          {tab === "timeline" && (
            <ol className="space-y-2 text-sm">
              ...
              <li>• Upgraded VIP — 02 Mar 2025</li>
              <li>• Last trade — today</li>
            </ol>
          )}
          {tab === "actions" && (
            <div className="space-y-2">
              <Btn variant="outline" onClick={() => toast("Opening full profile")}>
                Open Full Profile
              </Btn>
              <Btn variant="outline" onClick={() => toast("Note added")}>
                Add Note
              </Btn>
              <Btn variant="danger" onClick={() => toast.error("Client restricted")}>
                Restrict Client
              </Btn>
            </div>
          )}
