  ...
  head: () => ({
    meta: [
      { title: "Commissions & Rebates · Atlas CRM" },
      {
        name: "description",
        content:
          "IB commission, rebate, CPA, revenue share earnings, payouts, adjustments and disputes.",
      },
    ],
  }),
  component: () => (
...
const PARTNER = {
  ...
  tier: "Gold IB",
  status: "Active",
  model: "Hybrid (Rev Share + CPA)",
};

const SECTIONS = [
  { k: "summary", label: "Summary" },
  { k: "accounts", label: "Accounts Overview" },
  { k: "txn", label: "All Transactions" },
...
const KPIS = [
  ...
  { label: "Total Rebates Generated", value: "$18,450", sub: "Volume based", tone: "blue" },
...
const DISPUTES = [
  {
    id: "DSP-221",
    amount: "$3,450",
    reason: "Payment SLA breach",
    status: "Investigating",
    team: "IB Team",
    date: "22 May 2025",
    notes: "TXN-8799 commission generated but PAY-5498 still processing beyond 5-day SLA.",
  },
  {
    id: "DSP-208",
    amount: "$2,040",
    reason: "Hold not justified",
    status: "Resolved",
    team: "Compliance",
    date: "12 May 2025",
    notes: "Hold released after compliance review.",
  },
];
...
const TIMELINE = [
  ...
  { t: "22 May 2025 10:30", by: "IB Team", e: "Dispute DSP-221 raised — payment SLA breach" },
  {
    t: "20 May 2025 16:45",
    by: "Finance L1",
    e: "Adjustment ADJ-289 debit $1,240 — duplicate payment",
  },
...
const NOTES = [
  {
    txt: "VIP partner — confirmed rev share at 35% effective Q1 2025.",
    by: "Finance L2",
    date: "10 May 2025",
  },
  {
    txt: "Commission review pending — ACC-1077 David Chen commission on hold pending compliance review of trading activity.",
    by: "Compliance",
    date: "25 May 2025",
  },
  {
    txt: "Duplicate DSP-221 — investigation owner is IB Team Lead.",
    by: "IB Team",
    date: "22 May 2025",
  },
];
      ...
      <div className="mx-auto max-w-7xl space-y-6 px-6 py-6">
        ...
        {tab === "accounts" && (
          <TableSection
            ...
            sub="All individual client accounts tracking lots and earnings"
            cols={["Client", "Type", "Platform", "Lots", "Revenue", "Earnings", "Status"]}
            rows={COMMISSION_ACCOUNTS}
            render={(r) => (
              <tr
                key={r.acc}
                className="cursor-pointer hover:bg-gray-50"
                onClick={() => setDrawer({ kind: "account", data: r })}
              >
                {/* <Td className="font-medium">{r.acc}</Td> */}
                <Td>{r.client}</Td>
                <Td>
                ...
                <Td>{r.lots}</Td>
                <Td>{r.revenue}</Td>
                <Td className="font-semibold">{r.commission}</Td>
                <Td>
        ...
        {tab === "txn" && (
          <TableSection
            ...
            sub="Every payout transaction including Rev Share, CPA, and Rebates"
            cols={["Transaction ID", "Client", "Account", "Type", "Amount", "Status", "Date"]}
            rows={COMMISSION_TXNS}
            render={(r) => (
              <tr
                key={r.id}
                className="cursor-pointer hover:bg-gray-50"
                onClick={() => setDrawer({ kind: "txn", data: r })}
              >
        ...
        {tab === "revshare" && (
          <TableSection
            ...
            sub="Rev-share commissions by partner client"
            cols={[
              "Partner",
              "Rev Share %",
              "Gross Revenue",
            ...
            render={(r, i) => (
              <tr key={i} className="hover:bg-gray-50">
                ...
                <Td>{r.gross}</Td>
                <Td>{r.net}</Td>
                <Td className="font-semibold">{r.earn}</Td>
                <Td>{r.date}</Td>
              </tr>
            )}
          />
        ...
        {tab === "cpa" && (
          <TableSection
            title="CPA Earnings"
            sub="Cost-per-acquisition deals"
            cols={["CPA ID", "Client", "Qualified", "Per Client", "Earnings", "Status"]}
            rows={CPA}
            render={(r) => (
              <tr key={r.id} className="hover:bg-gray-50">
                ...
                <Td>{r.qualified}</Td>
                <Td>{r.perClient}</Td>
                <Td className="font-semibold">{r.earnings}</Td>
                <Td>
                  <Pill tone={statusTone(r.status)}>{r.status}</Pill>
                </Td>
              </tr>
        ...
        {tab === "hybrid" && (
          <TableSection
            title="Hybrid Earnings"
            sub="Combination of CPA + Revenue Share"
            cols={["ID", "Client", "CPA Portion", "Rev Share Portion", "Total", "Status"]}
            rows={HYBRID}
            render={(r) => (
              <tr key={r.id} className="hover:bg-gray-50">
                <Td className="font-medium">{r.id}</Td>
        ...
        {tab === "adjust" && (
          <TableSection
            ...
            sub="Credits & debits to commission"
            cols={["Adjustment ID", "Type", "Amount", "Reason", "By", "Date"]}
            rows={ADJUSTMENTS}
            render={(r) => (
              <tr key={r.id} className="hover:bg-gray-50">
                <Td className="font-medium">{r.id}</Td>
        ...
        {tab === "pending" && (
          <TableSection
            ...
            sub="Unpaid earnings awaiting payout"
            cols={["Payment ID", "Amount", "Due Date", "Method", "Status"]}
            rows={PENDING}
            render={(r) => (
              <tr
                key={r.id}
                className="cursor-pointer hover:bg-gray-50"
                onClick={() => setDrawer({ kind: "payment", data: r })}
              >
                <Td className="font-medium">{r.id}</Td>
                <Td className="font-semibold">{r.amount}</Td>
                <Td>{r.due}</Td>
            ...
            action={
              <div className="flex gap-2">
                <Btn onClick={() => notify("Selected payments approved")}>✓ Approve Selected</Btn>
        ...
        {tab === "history" && (
          <TableSection
            ...
            cols={["Payment ID", "Amount", "Method", "Date", "Status"]}
            rows={HISTORY}
            render={(r) => (
              <tr key={r.id} className="hover:bg-gray-50">
                <Td className="font-medium">{r.id}</Td>
        ...
        {tab === "contrib" && (
          <TableSection
            ...
            sub="Top revenue-generating clients"
            cols={["Client", "Account", "Lots", "Revenue Generated", "Commission Earned"]}
            rows={CONTRIB}
            render={(r, i) => (
              <tr key={i} className="hover:bg-gray-50">
                ...
                <Td className="font-semibold text-emerald-700">{r.commission}</Td>
              </tr>
            )}
          />
        )}
        {tab === "disputes" && (
          <TableSection
            ...
            cols={["Dispute ID", "Amount", "Reason", "Status", "Assigned Team", "Date"]}
            rows={DISPUTES}
            render={(r) => (
              <tr
                key={r.id}
                className="cursor-pointer hover:bg-gray-50"
                onClick={() => setDrawer({ kind: "dispute", data: r })}
              >
                <Td className="font-medium">{r.id}</Td>
                <Td className="font-semibold">{r.amount}</Td>
                <Td>{r.reason}</Td>
        ...
        {tab === "rules" && (
          <Section
            ...
            subtitle="Active commission configuration for this partner"
          >
        ...
        {tab === "timeline" && (
          <Section title="Timeline" subtitle="All commission lifecycle events">
        ...
        {tab === "notes" && (
          <Section
            title="Notes"
            subtitle="Internal commission notes"
            action={<Btn onClick={() => notify("Note added")}>+ Add</Btn>}
          >
        ...
        {tab === "audit" && (
          <TableSection
            title="Audit Trail"
            sub="All commission-related changes"
            cols={["Action", "By", "Entity", "Timestamp", "IP"]}
            rows={AUDIT}
            render={(r, i) => (
              <tr key={i} className="hover:bg-gray-50">
          ...
          {[
            ...
            <div key={r.l} className="mb-2">
              ...
              <div className="mt-1 h-1.5 w-full rounded bg-gray-100">
                <div className="h-1.5 rounded bg-gray-900" style={{ width: `${r.pct}%` }} />
              </div>
            </div>
          ))}
        ...
        <Section title="Payment Status" subtitle="Current month payout state">
          {[
            ...
            <div key={r.l} className="mb-3">
              ...
              <div className="mt-1 h-1.5 w-full rounded bg-gray-100">
                <div
                  className={`h-1.5 rounded ${r.tone === "green" ? "bg-emerald-500" : r.tone === "amber" ? "bg-amber-500" : "bg-rose-500"}`}
                  style={{ width: `${r.pct}%` }}
                />
              </div>
            </div>
      ...
      action={
        action ?? (
          <div className="flex gap-2">
            ...
            />
            <Btn variant="ghost" onClick={() => toast.success("Exported")}>
              Export
            </Btn>
          </div>
        )
    ...
    <div className="grid gap-6 md:grid-cols-3">
      <Section title="Safe Actions" subtitle="No approval required">
        <div className="space-y-2">
          {["View Earnings", "Export Report", "Add Note", "View Payment"].map((a) => (
            <Btn key={a} variant="ghost" onClick={() => notify(`${a} executed`)}>
              {a}
            </Btn>
          ))}
        </div>
      ...
      <Section title="Moderate Actions" subtitle="Standard approval">
        <div className="space-y-2">
          {["Approve Payment", "Adjust Commission", "Hold Payment", "Assign Review"].map((a) => (
            <Btn key={a} onClick={() => notify(`${a} executed`)}>
              {a}
            </Btn>
          ))}
        </div>
      ...
      <Section
        ...
        subtitle="Requires permission, reason, audit log, confirmation"
      >
        <div className="space-y-2">
          {["Reverse Commission", "Cancel Payment", "Hold Commission", "Freeze Earnings"].map(
            (a) => (
              <Btn key={a} variant="danger" onClick={() => notify(`${a} executed`)}>
                {a}
              </Btn>
            ),
          )}
...
function Drawer({
  ...
  const tabs =
    kind === "txn"
      ? ["overview", "source", "trading", "calc", "payment", "timeline", "notes", "audit"]
      : kind === "payment"
        ? ["overview", "method", "bank", "sources", "approval", "timeline"]
        : kind === "dispute"
          ? ["overview", "details", "timeline", "audit"]
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-gray-900/40" onClick={onClose}>
      ...
      >
        <div className="border-b border-gray-200 p-5">
          <div className="flex items-start justify-between gap-3">
            <div>
              <div className="flex gap-2">
                <Pill tone={statusTone(r.status || "")}>{r.status || "—"}</Pill>
                <Pill tone="blue">{kind}</Pill>
              </div>
              <h3 className="mt-2 text-lg font-semibold">
                {r.id || r.acc} — {r.client || r.name || ""}
              </h3>
        ...
        <div className="p-5 space-y-2">
          ...
          {t === "source" && kind === "txn" && (
            <>
              <KV k="Client Name" v={r.client} />
              <KV k="Account" v={r.acc} />
              <KV k="Platform" v="MT5" />
          ...
          {t === "trading" && kind === "txn" && (
            <>
              <KV k="Period" v={r.period} />
              <KV k="Lots Traded" v="842" />
              <KV k="Instruments" v="EURUSD, XAUUSD, US30" />
              <KV k="Trades" v="412" />
            </>
          )}
          {t === "calc" && kind === "txn" && (
            <>
              <KV k="Gross Revenue" v={r.gross} />
              <KV k="Net Revenue" v={r.net} />
              <KV k="Share Rate" v={r.rate} />
              <KV
                k="Commission"
                v={<span className="font-bold text-emerald-700">{r.amount}</span>}
              />
            </>
          )}
          {t === "payment" && (
            <>
              <KV k="Due Date" v={r.due || "—"} />
              <KV k="Method" v={r.method || "Bank Wire"} />
              <KV k="Linked Payment" v={r.payId || "—"} />
              <KV k="Currency" v="USD" />
            </>
          )}
          {t === "method" && (
            <>
              <KV k="Method" v={r.method} />
              <KV k="Currency" v="USD" />
              <KV k="Fee" v="$0" />
            </>
          )}
          {t === "bank" && (
            <>
              <KV k="Bank Name" v="HDFC Bank" />
              <KV k="Account" v="****4521" />
              <KV k="SWIFT" v="HDFCINBB" />
              <KV k="Beneficiary" v={PARTNER.name} />
            </>
          )}
          {t === "sources" && (
            <>
              <KV k="Source Txns" v="TXN-8821, TXN-8744" />
              <KV k="Period" v="May 2025" />
            </>
          )}
          {t === "approval" && (
            <>
              <KV k="Requested By" v="System" />
              <KV k="Approved By" v="Finance L2" />
              <KV k="Approved At" v="30 May 13:50" />
            </>
          )}
          {t === "details" && (
            <>
              ...
              {r.notes && (
                <div className="mt-3 rounded bg-amber-50 p-3 text-sm text-amber-900">{r.notes}</div>
              )}
            </>
          )}
          {t === "timeline" && (
            <ol className="space-y-2 text-sm">
              ...
              <li className="border-l-2 border-gray-300 pl-3">
                <div className="text-xs text-gray-500">Yesterday · Finance</div>
                <div>Updated status to {r.status}</div>
              </li>
            </ol>
          )}
          {t === "notes" && (
            <div className="rounded bg-gray-50 p-3 text-sm text-gray-700">No notes yet.</div>
          )}
          {t === "audit" && (
            <ol className="space-y-1 text-xs text-gray-600">
              <li>Created · System · Today 09:00</li>
              <li>Updated · Finance L2 · Today 14:20</li>
            </ol>
          )}
        </div>
